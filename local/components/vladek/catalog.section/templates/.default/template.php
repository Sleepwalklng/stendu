...
<div class="container">
    <div class="title">
        <span>Каталог</span> <div class="catalog__filter-open"><img src="<?=SITE_TEMPLATE_PATH?>/img/icon/sort.svg" alt=""></div>
    </div>
    <div class="catalog__wrapper">
        <div class="catalog__left">
            <div class="catalog__left-buttons">
                <div class="catalog__left-buttons-green">

                </div>
                <div class="catalog__left-buttons-close">
                    <img src="<?=SITE_TEMPLATE_PATH?>/img/greyxxx.svg" alt="">
                </div>
            </div>
            <form class="catalog-filters catalog-filters-ajax">
                <div class="catalog-filters__title">
                    <strong>выбрано</strong><button class="catalog-filters__clear-all catalog-choice__clear catalog-choice-clear-ajax">Очистить</button>
                </div>
                <div class="catalog-filters__block catalog-choice catalog-choice__item-ajax">

                </div>
                <?php foreach ($filter as $key => $val): ?>
                    <div class="catalog-filters__block-toggle ">
                        <button class="catalog-filters__toggle"><?=$key?></button>
                        <ul class="catalog-filters__bottom">
                            <?php foreach ($val as $k => $v): ?>
                                <li class="catalog-filters__item catalog-filters-item-ajax">
                                <label class="custom-checkbox" data-text="<?=$v['NAME']?>">
                                    <input <?php if(in_array($v['CODE'], $_GET['FILTER'])) echo 'checked';?> type="checkbox" class="custom-checkbox__input checkbox-filter-ajax visually-hidden" name="FILTER[]" value="<?=$v['CODE']?>">
                                    <span class="custom-checkbox__text"><?=$v['NAME']?></span>
                                </label>
                            </li>
                            <?php endforeach;?>
                        </ul>
                ...
                <?php endforeach;?>
                <input type="hidden" class="current-page-id" name="PAGE_ID" value="0">
            </form>
        </div>
