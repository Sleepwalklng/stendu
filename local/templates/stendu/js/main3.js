...
$('.categories-modal__btn').on('click', function () {
	...
	for (let i = 0; i < arr.length; i++) {
		if (arr[i] == $('.categories-modal__btn--active').find('.link__text')[0]) {
			...
			$('.categories-modal__list:nth-child(' + (i + 1) + ')').addClass('categories-modal__list--active');
		}
	}
});
...
if ($(window).width() < 769) {
	...
	for (let i = 1; i <= $('.data__categories--transport .data__categories-item').length; i++) {
		if (i == 4) {
			$('.data__categories--transport .data__categories-item:nth-child(' + i + ')').find('.data__text').text('Комтранс');
		} else {
	...
	const mounthsShort = ['янв', 'фев', 'мар', 'апр', 'мая', 'июн', 'июл', 'авг', 'сен', 'окт', 'ноя', 'дек'];

	$.each(arr, function (index, elem) {
		for (let i = 0; i < mounths.length; i++) {
			if (elem.querySelector('.cart__date').innerHTML.includes(mounths[i])) {
				elem.querySelector('.cart__date').innerHTML = elem.querySelector('.cart__date').innerHTML.replace(mounths[i], mounthsShort[i])
			}
		}
	});
}
...
if ($(".card__cost-polzunok--time .cost-polzunok").html() != null) {
	...
	$(".card__cost-polzunok--time .cost-polzunok__input-right").val($(".card__cost-polzunok--time .cost-polzunok").slider("values", 1) + " лет");
...
if ($(".card__cost-polzunok--long .cost-polzunok").html() != null) {
	...
	$(".card__cost-polzunok--long .cost-polzunok__input-right").text($(".card__cost-polzunok--long .cost-polzunok").slider("values", 1) + " лет");
