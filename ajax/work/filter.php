...
// Получение сферы услуг
const MY_HL_BLOCK_ID_FIELD_ACTIVITY = 29;
$entity_data_class_field_activity = GetEntityDataClass(MY_HL_BLOCK_ID_FIELD_ACTIVITY);
$rsUslugi = $entity_data_class_field_activity::getList(array( "select" => array("*"),
    "order" => array("ID" => "DESC"),));
while ($el = $rsUslugi->fetch()) {
    if ($_POST['UF_FIELDACTIVITY'] == $el['UF_NAME']){
        $mainCategoria = $el['ID'];
    }
};
...
if(!empty($_POST['exp'])){
    $exp = $_POST['exp'];
    foreach ($arExp as $expItem){
        if($exp == $expItem['VALUE']){
            $expId = $expItem['ID'];
        }
    }
}

const MY_HL_BLOCK_ID_USLUGIS = 11;
...
$list_array['filter']['UF_STATUS'] = 34;
if($mainCategoria) {
    $list_array['filter']['UF_FIELDACTIVITY'] = $mainCategoria;
}
if ($categoria) {
    $list_array['filter']['UF_TYPE_WORK'] = $categoria;
...
if ($expId) {
    $list_array['filter']['UF_EXPERIENCE'] = $expId;
}
if (!empty($_POST['candidat'])) {
    $list_array['filter']['UF_CANDIDAT'] = $_POST['candidat'];
}
...
<ul class="announcements__items">
    <? foreach ($arUslugi as $arItem): ?>
        ...
        <li class="announcements__item cart <? if (in_array($arItem['ID'], $resultFav)) {
            ...
            <a href="/rabota/<?= $arItem['UF_SESSION_CODE'] ?>/">
                <div class="cart__top">
